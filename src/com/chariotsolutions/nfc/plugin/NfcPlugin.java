...
public class NfcPlugin extends Plugin {
    ...
    public PluginResult execute(String action, JSONArray data, String callbackId) {
        ...
        if (action.equalsIgnoreCase(REGISTER_MIME_TYPE)) {
            ...
            if (((DroidGap) ctx).getIntent() == null) {  // TODO remove this and handle LostTag
            ...
            Log.d(TAG, "Enabling plugin " + ((DroidGap) ctx).getIntent());
    ...
    private void createPendingIntent() {
        if (pendingIntent == null) {
            Intent intent = new Intent((DroidGap) ctx, ctx.getClass());
    ...
    private void startNfc() {
        ...
        this.ctx.runOnUiThread(new Runnable() {
            public void run() {
                NfcAdapter.getDefaultAdapter((DroidGap) ctx).enableForegroundDispatch(
                        ctx, getPendingIntent(), getIntentFilters(), getTechLists());
                if (p2pMessage != null) {
                    NfcAdapter.getDefaultAdapter((DroidGap) ctx).enableForegroundNdefPush(ctx, p2pMessage);
                }
            }
        });
    }
    ...
    private void stopNfc() {
        this.ctx.runOnUiThread(new Runnable() {
            public void run() {
                NfcAdapter.getDefaultAdapter((DroidGap) ctx).disableForegroundDispatch(ctx);
                NfcAdapter.getDefaultAdapter((DroidGap) ctx).disableForegroundNdefPush(ctx);
            }
        });
    }
    ...
    public void parseMessage() {
        Log.d(TAG, "parseMessage " + ((DroidGap) ctx).getIntent());
        Intent intent = ((DroidGap) ctx).getIntent();
        ...
        ((DroidGap) ctx).setIntent(new Intent());
    }
    ...
    private boolean recycledIntent() { // TODO this is a kludge, find real solution
        ...
        int flags = ((DroidGap) ctx).getIntent().getFlags();
    ...
    public void onPause(boolean multitasking) {
        Log.d(TAG, "onPause " + ((DroidGap) ctx).getIntent());
    ...
    public void onResume(boolean multitasking) {
        Log.d(TAG, "onResume " + ((DroidGap) ctx).getIntent());
